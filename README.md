...
最新の改善により、血統合議複勝モデル（`pedigree_mid_20_stable`）と新設した高回収3連複ルール（`trio_high_roi`: 3頭予測順位4位以内、オッズ`[20.0,200.0)`かつ芝1800m〜2600m限定）を組み合わせたポートフォリオ`pedigree_trio_roi`が最も優れた成績を収めました。購入率は26.08%（714レース、1266点）に調整され、**ROI 148.14%、最低fold ROI 122.94%**と極めて安定した結果を示しました。また、2つの高回収3連複ルールを組み合わせる`trio_double_roi`は、購入率26.49%で最大**ROI 149.77%**（最低fold ROI 98.30%）を達成しました。
